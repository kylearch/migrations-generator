...
namespace KyleArch\MigrationGenerator\Generators;
...
class FieldGenerator
{
    ...
    {
        try {
            $result = DB::table('information_schema.columns')->where('table_schema', $this->database)->where('table_name', $table)->where('data_type', 'enum')->get(['column_name', 'column_type']);
            if ($result) {
                return $result;
            } else {
                return [];
        ...
        foreach ($columns as $column) {
            ...
            if (in_array($type, ['tinyInteger', 'smallInteger', 'integer', 'bigInteger'])) {
                ...
                if ($name == 'deleted_at' and $nullable) {
                    ...
                    $type     = 'softDeletes';
                    $name     = '';
                } else if ($name == 'created_at' and isset($fields['updated_at'])) {
                    $fields['updated_at'] = ['field' => '', 'type' => 'timestamps'];
                    continue;
                } else if ($name == 'updated_at' and isset($fields['created_at'])) {
                    $fields['created_at'] = ['field' => '', 'type' => 'timestamps'];
                    continue;
                }
            } else if (in_array($type, ['decimal', 'float', 'double'])) {
    ...
    protected function getDefault($default, &$type)
    {
        ...
        } else if (in_array($type, ['string', 'text']) or !is_numeric($default)) {
